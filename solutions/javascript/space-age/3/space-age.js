...
export const age = (planet, sec) => {
  ...
  const [firstLetter, ...restLetters] = planet;
  const planetOrbital = ORBITAL[[firstLetter.toUpperCase(), ...restLetters].join('')];
  if (!planetOrbital) {
    throw new Error('not a planet') 
  } else {
    return Number(Number(sec/(planetOrbital*31557600)).toFixed(2));
  }
};
